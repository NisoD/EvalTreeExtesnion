#!/usr/bin/env python3
"""
DOVE-Based Sunburst with Horizontal Central Text

Creates sunburst visualizations with:
- Horizontal text orientation in central circle
- Shortened capability names (4-7 key words)
- Red = Critical/Weak, Blue = Strong
- Interactive features and proper color coding
"""
...
def extract_flat_weakness_data(node, dove_scores, level=0, max_levels=3):
    ...
    def traverse(current_node, current_level, parent_id=""):
        ...
        if subtree_scores and len(subtree_scores) >= 3:
            ...
            data.append(
                {
                    ...
                    "weakness_level": (
                        ...
                        if mean_score < 0.3
                            ...
                            "High"
                            if mean_score < 0.5
                            else "Moderate" if mean_score < 0.7 else "Low"
                        )
                    ),
                }
            )
            ...
            if "subtrees" in current_node and isinstance(
                current_node["subtrees"], list
            ):
...
def create_horizontal_sunburst(data, title="DOVE Weakness Profile", top_n=50):
    """Create a sunburst visualization with horizontal central text"""
    ...
    for i, item in enumerate(sorted_data[:5]):
        print(
            f"  {i+1}. {item['labels']} (Score: {item['dove_score']}, Questions: {item['values']})"
        )

    # Create sunburst with horizontal text orientation
    fig = go.Figure(
        go.Sunburst(
            ids=[d["ids"] for d in sorted_data],
            ...
            customdata=[
                [
                    ...
                    d["weakness_level"],
                    (
                        d["full_capability"][:60] + "..."
                        if len(d["full_capability"]) > 60
                        else d["full_capability"]
                    ),
                ]
                for d in sorted_data
            ],
            # TEXT ORIENTATION SETTINGS - KEY ADDITION
            textfont=dict(size=11, family="Arial"),
            insidetextorientation='horizontal',  # Makes central text horizontal
            texttemplate='%{label}',
    ...
    fig.update_layout(
        ...
        height=900,
        showlegend=False,
        # Additional layout settings for better text display
    ...
    for level in ["Critical", "High", "Moderate", "Low"]:
        if level in by_level:
            ...
            for i, item in enumerate(items[:10], 1):
                capability = (
                    item["full_capability"][:50] + "..."
                    if len(item["full_capability"]) > 50
                    else item["full_capability"]
                )
                print(f"{i:2d}. {item['labels']} -> {capability}")
                print(
                    f"    DOVE Score: {item['dove_score']} | Questions: {item['values']}"
                )
...
def main():
    ...
    print("Creating DOVE Sunburst with Horizontal Central Text...")
    ...
    try:
        ...
        # Create horizontal sunburst (top 50)
        print("Creating horizontal sunburst...")
        main_fig, sorted_data = create_horizontal_sunburst(weakness_data, top_n=50)
        if main_fig:
            main_fig.write_html("horizontal_dove_sunburst.html")
            print("✅ Horizontal sunburst saved to horizontal_dove_sunburst.html")

        # Create focused horizontal version (top 25)
        print("Creating focused horizontal sunburst...")
        focused_fig, focused_data = create_horizontal_sunburst(
            weakness_data, title="DOVE Critical Weaknesses (Horizontal)", top_n=25
        )
        if focused_fig:
            focused_fig.write_html("horizontal_dove_focused_sunburst.html")
            print("✅ Focused horizontal sunburst saved to horizontal_dove_focused_sunburst.html")
        ...
        print("\n" + "=" * 80)
        print("HORIZONTAL SUNBURST VISUALIZATION COMPLETE!")
        print("=" * 80)
        print("Generated files:")
        print("  • horizontal_dove_sunburst.html - Top 50 weakest (horizontal text)")
        print("  • horizontal_dove_focused_sunburst.html - Top 25 most critical")

        print("\n🎯 Key Features:")
        print("  • Horizontal text orientation in central circle")
        print("  • Shortened labels (4-7 key words)")
        print("  • Red = Critical/Weak, Blue = Strong")
        print("  • Interactive hover details")

        print("\n📊 Open horizontal_dove_sunburst.html in your browser!")
